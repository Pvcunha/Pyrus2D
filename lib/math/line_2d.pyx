...
cdef class Line2D:
    ...
    def dist(self, p: Vector2D):
        return fabs(
            (self._a * p.x() + self._b * p.y() + self._c) / sqrt(self._a * self._a + self._b * self._b))
    ...
    def isParallel(self, other):
        return fabs(self._a * other.b() - other.a() * self._b) < EPSILON
    ...
    def perpendicular_bisector(point1, point2):
        if fabs(point2.x - point1.x) < EPSILON and fabs(point2.y - point1.y) < EPSILON:
